...
class RequestIdentityWebFilterTests {
	...
	void requestIdentityHeader() {
		MockServerHttpRequest request = MockServerHttpRequest.get(V2_API_PATH_PATTERN)
		...
		webFilter.filter(this.exchange, this.chain).block();
		assertThat(this.exchange.getResponse().getHeaders().getFirst(ServiceBrokerRequest.REQUEST_IDENTITY_HEADER))
			.isEqualTo("request-id");
	}

	@Test
	void requestIdentityHeaderMissing() {
		MockServerHttpRequest request = MockServerHttpRequest.get(V2_API_PATH_PATTERN).build();
		this.exchange = MockServerWebExchange.from(request);
		openMocks(this);
		given(this.chain.filter(this.exchange)).willReturn(Mono.empty());
		RequestIdentityWebFilter webFilter = new RequestIdentityWebFilter();
