...
class CatalogControllerIntegrationTests {
	...
	void catalogIsRetrieved() throws Exception {
		MvcResult mvcResult = this.mockMvc.perform(get("/v2/catalog").accept(MediaType.APPLICATION_JSON))
	...
	void catalogIsRetrievedWithPlatformInstanceId() throws Exception {
		MvcResult mvcResult = this.mockMvc.perform(get("/123/v2/catalog").accept(MediaType.APPLICATION_JSON))
			.andExpect(request().asyncStarted())
			.andReturn();
		assertResult(mvcResult);
	}
	...
	void cachedCatalogWithSameEtagIsRetrieved() throws Exception {
		...
		MvcResult mvcResult = this.mockMvc
			.perform(get("/v2/catalog").header(HttpHeaders.IF_NONE_MATCH, "12345").accept(MediaType.APPLICATION_JSON))
			.andExpect(request().asyncStarted())
			.andReturn();
	...
	void catalogWithDifferentEtagIsRetrieved() throws Exception {
		...
		MvcResult mvcResult = this.mockMvc
			.perform(get("/v2/catalog").header(HttpHeaders.IF_NONE_MATCH, "22221").accept(MediaType.APPLICATION_JSON))
			.andExpect(request().asyncStarted())
			.andReturn();
	...
	private ResultActions assertResult(MvcResult mvcResult) throws Exception {
		List<String> features = (List<String>) this.serviceDefinition.getMetadata().get("features");
		...
		resultActions.andDo(print())
			...
			.andExpect(jsonPath("$.services[*].id", contains(this.serviceDefinition.getId())))
			.andExpect(jsonPath("$.services[*].name", contains(this.serviceDefinition.getName())))
			.andExpect(jsonPath("$.services[*].description", contains(this.serviceDefinition.getDescription())))
			.andExpect(jsonPath("$.services[*].bindable", contains(this.serviceDefinition.isBindable())))
			.andExpect(jsonPath("$.services[*].requires[*]",
					containsInAnyOrder(SERVICE_REQUIRES_SYSLOG_DRAIN.toString(),
