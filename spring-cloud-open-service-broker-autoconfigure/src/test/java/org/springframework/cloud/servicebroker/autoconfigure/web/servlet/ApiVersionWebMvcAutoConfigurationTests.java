...
class ApiVersionWebMvcAutoConfigurationTests {
	...
	void apiVersionCheckIsDisabled() {
		webApplicationContextRunner().withUserConfiguration(ServicesConfiguration.class)
			...
			.run((context) -> assertThat(context).doesNotHaveBean(BrokerApiVersion.class));
	}
	...
	private WebApplicationContextRunner webApplicationContextRunner() {
		return new WebApplicationContextRunner().withConfiguration(AutoConfigurations
			.of(ApiVersionWebMvcAutoConfiguration.class, ApiVersionWebFluxAutoConfiguration.class));
	}
