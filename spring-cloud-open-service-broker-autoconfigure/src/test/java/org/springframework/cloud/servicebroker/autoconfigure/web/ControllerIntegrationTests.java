...
public abstract class ControllerIntegrationTests {
	...
	protected void setupCatalogService() {
		given(this.catalogService.getServiceDefinition(isNull())).willReturn(Mono.empty());
		given(this.catalogService.getServiceDefinition(eq(this.serviceDefinition.getId())))
			.willReturn(Mono.just(this.serviceDefinition));
	}
