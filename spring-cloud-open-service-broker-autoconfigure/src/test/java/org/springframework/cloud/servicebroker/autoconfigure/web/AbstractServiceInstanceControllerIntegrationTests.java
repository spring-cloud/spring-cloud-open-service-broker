...
public abstract class AbstractServiceInstanceControllerIntegrationTests extends ControllerIntegrationTests {
	...
	public void setUpCommonFixtures() {
		...
		this.updateRequestBody = JsonUtils
			.toJson(UpdateServiceInstanceRequest.builder().serviceDefinitionId(serviceDefinition.getId()).build());
	...
	protected String buildCreateUpdateUrl(String platformInstanceId, boolean asyncAccepted) {
		return buildBaseUrl(platformInstanceId).path(SERVICE_INSTANCE_ID)
	...
	protected String buildGetUrl(String platformInstanceId, String serviceDefinitionId, String planId,
		...
		UriComponentsBuilder builder = buildBaseUrl(platformInstanceId).path(SERVICE_INSTANCE_ID)
			.queryParam("accepts_incomplete", asyncAccepted);
	...
	protected String buildDeleteUrl(String platformInstanceId, boolean asyncAccepted) {
		return buildBaseUrl(platformInstanceId).path(SERVICE_INSTANCE_ID)
			.queryParam("service_id", this.serviceDefinition.getId())
	...
	protected String buildLastOperationUrl(String platformInstanceId) {
		return buildBaseUrl(platformInstanceId).pathSegment(SERVICE_INSTANCE_ID, "last_operation")
			.queryParam("operation", "working")
			.toUriString();
	}

	protected UriComponentsBuilder buildBaseUrl(String platformInstanceId) {
		return UriComponentsBuilder.fromPath("//").path(platformInstanceId).path(SERVICE_INSTANCES_ROOT_PATH);
	}
	...
	protected CreateServiceInstanceRequest verifyCreateServiceInstance() {
		...
		Mockito.verify(this.serviceInstanceService).createServiceInstance(argumentCaptor.capture());
	...
	protected GetServiceInstanceRequest verifyGetServiceInstance() {
		...
		Mockito.verify(this.serviceInstanceService).getServiceInstance(argumentCaptor.capture());
		return argumentCaptor.getValue();
	}
	...
	protected DeleteServiceInstanceRequest verifyDeleteServiceInstance() {
		...
		Mockito.verify(this.serviceInstanceService).deleteServiceInstance(argumentCaptor.capture());
		return argumentCaptor.getValue();
	}
	...
	protected UpdateServiceInstanceRequest verifyUpdateServiceInstance() {
		...
		Mockito.verify(this.serviceInstanceService).updateServiceInstance(argumentCaptor.capture());
		return argumentCaptor.getValue();
	}
	...
	protected GetLastServiceOperationRequest verifyLastOperation() {
		...
		Mockito.verify(this.serviceInstanceService).getLastOperation(argumentCaptor.capture());
		return argumentCaptor.getValue();
	}
