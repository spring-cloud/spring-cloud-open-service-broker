...
class ServiceInstanceBindingControllerIntegrationTests
	...
	void createBindingToAppWithoutAsyncAndHeadersSucceeds(CapturedOutput output) throws Exception {
		...
		this.client.put()
			.uri(buildCreateUrl(PLATFORM_INSTANCE_ID, false))
	...
	void createBindingToAppWithAsyncAndHeadersSucceeds() throws Exception {
		...
		this.client.put()
			.uri(buildCreateUrl(PLATFORM_INSTANCE_ID, true))
			.contentType(MediaType.APPLICATION_JSON)
			.bodyValue(createRequestBody)
			.header(API_INFO_LOCATION_HEADER, API_INFO_LOCATION)
			.header(ORIGINATING_IDENTITY_HEADER, buildOriginatingIdentityHeader())
	...
	void createBindingToAppWithExistingSucceeds() {
		...
		this.client.put()
			.uri(buildCreateUrl())
			.contentType(MediaType.APPLICATION_JSON)
			.bodyValue(this.createRequestBody)
			.accept(MediaType.APPLICATION_JSON)
			.exchange()
			.expectStatus()
			.isOk();
	...
	void createBindingWithUnknownServiceDefinitionIdFails() {
		...
		this.client.put()
			...
			.jsonPath("$.description")
			.isNotEmpty()
			.consumeWith((result) -> assertDescriptionContains(result, this.serviceDefinition.getId()));
	}
	...
	void createBindingWithDuplicateIdFails() {
		...
		this.client.put()
			...
			.jsonPath("$.description")
			.isNotEmpty()
			.consumeWith((result) -> assertDescriptionContains(result, String
	...
	void deleteBindingWithoutAsyncAndHeadersSucceeds(CapturedOutput output) throws Exception {
		...
		this.client.delete()
			.uri(buildDeleteUrl(PLATFORM_INSTANCE_ID, false))
			.header(API_INFO_LOCATION_HEADER, API_INFO_LOCATION)
			.header(ORIGINATING_IDENTITY_HEADER, buildOriginatingIdentityHeader())
			.exchange()
			.expectStatus()
	...
	void deleteBindingWithUnknownServiceDefinitionIdFails() {
		...
		this.client.delete().uri(buildDeleteUrl()).exchange().expectStatus().isBadRequest();
	}
