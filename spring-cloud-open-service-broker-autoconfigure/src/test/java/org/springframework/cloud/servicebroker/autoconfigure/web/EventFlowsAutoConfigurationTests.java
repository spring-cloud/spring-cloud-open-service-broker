...
class EventFlowsAutoConfigurationTests {
	...
	void servicesAreCreatedWithAlternateFlowBeansConfiguration() {
		this.contextRunner.withUserConfiguration(AlternateEventFlowRegistryBeansConfiguration.class)
			.run(this::assertBeans);
	}

	@Test
	void createInstanceEventFlowBeansAreConfigured() {
		this.contextRunner.withUserConfiguration(CreateInstanceEventFlowBeansConfiguration.class).run((context) -> {
			assertBeans(context);
			CreateServiceInstanceEventFlowRegistry registry = context
	...
	void updateInstanceEventFlowBeansAreConfigured() {
		this.contextRunner.withUserConfiguration(UpdateInstanceEventFlowBeansConfiguration.class).run((context) -> {
			assertBeans(context);
			UpdateServiceInstanceEventFlowRegistry registry = context
	...
	void deleteInstanceEventFlowBeansAreConfigured() {
		this.contextRunner.withUserConfiguration(DeleteInstanceEventFlowBeansConfiguration.class).run((context) -> {
			assertBeans(context);
			DeleteServiceInstanceEventFlowRegistry registry = context
	...
	void asyncOperationEventFlowBeansAreConfigured() {
		this.contextRunner.withUserConfiguration(AsyncOperationServiceInstanceEventFlowBeansConfiguration.class)
			.run((context) -> {
				assertBeans(context);
				AsyncOperationServiceInstanceEventFlowRegistry registry = context
	...
	void createInstanceBindingEventFlowBeansAreConfigured() {
		this.contextRunner.withUserConfiguration(CreateInstanceBindingEventFlowBeansConfiguration.class)
			.run((context) -> {
				assertBeans(context);
				CreateServiceInstanceBindingEventFlowRegistry registry = context
	...
	void deleteInstanceBindingEventFlowBeansAreConfigured() {
		this.contextRunner.withUserConfiguration(DeleteInstanceBindingEventFlowBeansConfiguration.class)
			.run((context) -> {
				assertBeans(context);
				DeleteServiceInstanceBindingEventFlowRegistry registry = context
	...
	void asyncBindingOperationEventFlowBeansAreConfigured() {
		this.contextRunner.withUserConfiguration(AsyncOperationServiceInstanceBindingEventFlowBeansConfiguration.class)
			.run((context) -> {
				assertBeans(context);
				AsyncOperationServiceInstanceBindingEventFlowRegistry registry = context
	...
	private void assertBeans(AssertableApplicationContext context) {
		assertThat(context).getBean(CreateServiceInstanceEventFlowRegistry.class).isNotNull();

		assertThat(context).getBean(UpdateServiceInstanceEventFlowRegistry.class).isNotNull();

		assertThat(context).getBean(DeleteServiceInstanceEventFlowRegistry.class).isNotNull();

		assertThat(context).getBean(AsyncOperationServiceInstanceEventFlowRegistry.class).isNotNull();

		assertThat(context).getBean(CreateServiceInstanceBindingEventFlowRegistry.class).isNotNull();

		assertThat(context).getBean(DeleteServiceInstanceBindingEventFlowRegistry.class).isNotNull();

		assertThat(context).getBean(AsyncOperationServiceInstanceBindingEventFlowRegistry.class).isNotNull();

		assertThat(context).getBean(EventFlowRegistries.class).isNotNull();
	...
	private void assertEventFlowBeans(EventFlowRegistry<?, ?, ?, ?, ?> registry, int initializationFlowCount,
		...
		List<?> initializationFlows = (List<?>) ReflectionTestUtils.getField(registry, "initializationFlows");
	...
	protected static class AlternateEventFlowRegistryBeansConfiguration {
		...
		public CreateServiceInstanceBindingEventFlowRegistry createInstanceBindingRegistry() {
			return new CreateServiceInstanceBindingEventFlowRegistry(new ArrayList<>(), new ArrayList<>(),
					new ArrayList<>());
		}

		@Bean
		public DeleteServiceInstanceBindingEventFlowRegistry deleteInstanceBindingRegistry() {
			return new DeleteServiceInstanceBindingEventFlowRegistry(new ArrayList<>(), new ArrayList<>(),
					new ArrayList<>());
		}
