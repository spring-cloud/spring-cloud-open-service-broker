...
class ApiVersionInterceptorIntegrationTests {
	...
	void noHeaderSent() throws Exception {
		mockWithExpectedVersion().perform(get(CATALOG_PATH).accept(MediaType.APPLICATION_JSON))
	...
	void incorrectHeaderSent() throws Exception {
		mockWithExpectedVersion()
			.perform(get(CATALOG_PATH).header(BrokerApiVersion.DEFAULT_API_VERSION_HEADER, "wrong-version")
	...
	void matchingHeaderSent() throws Exception {
		setUpCatalogMocks();
		mockWithExpectedVersion()
			.perform(get(CATALOG_PATH).header(BrokerApiVersion.DEFAULT_API_VERSION_HEADER, "expected-version")
				.accept(MediaType.APPLICATION_JSON))
	...
	void anyHeaderNotSent() throws Exception {
		setUpCatalogMocks();
		mockWithDefaultVersion().perform(get(CATALOG_PATH).accept(MediaType.APPLICATION_JSON))
			.andExpect(status().isOk());
	}
	...
	void anyHeaderSent() throws Exception {
		...
		mockWithDefaultVersion()
			.perform(get(CATALOG_PATH).header(BrokerApiVersion.DEFAULT_API_VERSION_HEADER, "ignored-version")
				.accept(MediaType.APPLICATION_JSON))
			.andExpect(status().isOk());
	}
