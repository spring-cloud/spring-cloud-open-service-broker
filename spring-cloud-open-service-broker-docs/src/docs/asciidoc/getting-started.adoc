...
While spring-cloud-open-service-broker uses reactive return types in its API, both blocking web stack (such as tomcat/jetty using `spring-boot-starter-webmvc`) or non blocking web stack (such as netty/undertow using `spring-boot-starter-webflux`) are supported. See respective acceptance tests in modules `spring-cloud-open-service-broker-acceptance-webmvc` and `spring-cloud-open-service-broker-acceptance-webflux`.
