...
A service broker can also return `maintenance_info` and `metadata` for the service instance in the {scosb-api}org/springframework/cloud/servicebroker/model/instance/GetServiceInstanceResponse.html[`GetServiceInstanceResponse`], using its `maintenanceInfo` and `metadata` properties.

The `metadata` object can carry `attributes`, broker-specific key-value pairs generated by the broker that may imply behavior changes by the platform, in addition to the existing `labels` property, using the {scosb-api}org/springframework/cloud/servicebroker/model/instance/ServiceInstanceMetadata.html[`ServiceInstanceMetadata`] object.
