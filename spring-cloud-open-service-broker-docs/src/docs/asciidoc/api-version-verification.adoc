...
Platforms are required to provide an https://github.com/openservicebrokerapi/servicebroker/blob/v2.17/spec.md#api-version-header[HTTP header] in each call to the Open Service Broker API, to indicate the version of the API specification that the platform supports.
...
[source,bash,%autofit]
----
spring.cloud.openservicebroker.apiVersion=2.17
----
====
