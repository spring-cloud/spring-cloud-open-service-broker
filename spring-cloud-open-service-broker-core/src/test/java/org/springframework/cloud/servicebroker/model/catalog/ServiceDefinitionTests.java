...
class ServiceDefinitionTests {
	...
	void serviceDefinitionWithAllFieldsIsSerializedToJson() {
		...
		ServiceDefinition serviceDefinition = ServiceDefinition.builder()
			...
			.metadata(metadata)
			.requires(SERVICE_REQUIRES_ROUTE_FORWARDING, SERVICE_REQUIRES_SYSLOG_DRAIN, SERVICE_REQUIRES_VOLUME_MOUNT)
		...
		assertThat(serviceDefinition.getRequires()).contains(SERVICE_REQUIRES_ROUTE_FORWARDING.toString(),
				SERVICE_REQUIRES_SYSLOG_DRAIN.toString(), SERVICE_REQUIRES_VOLUME_MOUNT.toString(), "another_requires");
		assertThat(serviceDefinition.getMetadata()).contains(entry("field1", "value1"), entry("field2", "value2"),
				entry("field3", "value3"), entry("field4", "value4"));
		...
		assertThat(json).hasListAtPath("$.requires[*]")
			.contains(SERVICE_REQUIRES_ROUTE_FORWARDING.toString(), SERVICE_REQUIRES_SYSLOG_DRAIN.toString(),
					SERVICE_REQUIRES_VOLUME_MOUNT.toString(), "another_requires");
