...
class CreateServiceInstanceBindingRequestTests {
	...
	void requestWithAllValuesIsBuilt() {
		...
		Context originatingIdentity = PlatformContext.builder().platform("test-platform").build();
	...
	void requestIsDeserializedFromJson() {
		CreateServiceInstanceBindingRequest request = JsonUtils.readTestDataFile("bindRequest.json",
	...
	void minimalRequiredRequestIsDeserializedFromJson() {
		CreateServiceInstanceBindingRequest request = JsonUtils
			.readTestDataFile("bindRequestWithOnlyRequiredFields.json", CreateServiceInstanceBindingRequest.class);

		// ensure required fields are present
		assertThat(request.getServiceDefinitionId()).isEqualTo("test-service-id");
		assertThat(request.getPlanId()).isEqualTo("test-plan-id");

		// ensure default value for missing non required fields is consistent with default
		// value provided by the builder
	...
	void minimalRequestMatchesWithJsonRoundTrip() {
		...
		CreateServiceInstanceBindingRequest fromJson = fromJson(json, CreateServiceInstanceBindingRequest.class);
	...
	void requestSerializesToJsonExcludingTransients() {
		CreateServiceInstanceBindingRequest request = CreateServiceInstanceBindingRequest.builder()
			...
			.originatingIdentity(PlatformContext.builder().platform("sample-platform").build())
			.asyncAccepted(true)
			.serviceDefinitionId("definition-id")
		...
		// Fields present in OSB Json body should be present, but no unspecified optional
		// ones.
		JsonPathAssert.assertThat(json).hasPath("$.service_id").isEqualTo("definition-id");

		// other fields mapped outside of json body (typically http headers or request
		// paths)
		// should be excluded
	...
	void equalsAndHashCode() {
		EqualsVerifier.forClass(CreateServiceInstanceBindingRequest.class)
