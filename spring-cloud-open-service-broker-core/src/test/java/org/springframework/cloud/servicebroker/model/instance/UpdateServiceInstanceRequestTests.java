...
class UpdateServiceInstanceRequestTests {
	...
	void requestWithAllValuesIsBuilt() {
		...
		UpdateServiceInstanceRequest request = UpdateServiceInstanceRequest.builder()
			...
			.previousValues(PreviousValues.builder()
				...
				.maintenanceInfo(MaintenanceInfo.builder().version("1.1.0").description("Patch for CVE-x").build())
				.build())
			.context(context)
		...
		assertThat(request.getPreviousValues().getMaintenanceInfo())
			.isEqualTo(new MaintenanceInfo("1.1.0", "Patch for CVE-x"));
	...
	void requestIsDeserializedFromJson() {
		UpdateServiceInstanceRequest request = JsonUtils.readTestDataFile("updateRequest.json",
	...
	void equalsAndHashCode() {
		EqualsVerifier.forClass(UpdateServiceInstanceRequest.class)
