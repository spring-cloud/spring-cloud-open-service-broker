...
class CreateServiceInstanceRequestTests {
	...
	void requestWithAllValuesIsBuilt() {
		...
		CreateServiceInstanceRequest request = CreateServiceInstanceRequest.builder()
			...
			.maintenanceInfo(new MaintenanceInfo("1.1.0", "Patch for CVE-x"))
			.build();
		...
		assertThat(request.getMaintenanceInfo()).isEqualTo(new MaintenanceInfo("1.1.0", "Patch for CVE-x"));
	}
	...
	void serializesAccordingToOsbSpecs() {
		...
		Context context = CloudFoundryContext.builder().organizationGuid("org-guid").spaceGuid("space-guid").build();

		Context originatingIdentity = CloudFoundryContext.builder().property("user_id", "user-id").build();
	...
	void requestIsDeserializedFromJson() {
		CreateServiceInstanceRequest request = JsonUtils.readTestDataFile("createRequest.json",
	...
	void equalsAndHashCode() {
		EqualsVerifier.forClass(CreateServiceInstanceRequest.class)
